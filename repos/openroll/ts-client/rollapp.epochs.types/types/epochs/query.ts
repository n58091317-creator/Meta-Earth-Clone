...
export class QueryClientImpl implements Query {
  ...
  EpochInfos(request: QueryEpochsInfoRequest): Promise<QueryEpochsInfoResponse> {
    ...
    const promise = this.rpc.request("rollapp.epochs.types.Query", "EpochInfos", data);
    return promise.then((data) => QueryEpochsInfoResponse.decode(new _m0.Reader(data)));
  }

  CurrentEpoch(request: QueryCurrentEpochRequest): Promise<QueryCurrentEpochResponse> {
    const data = QueryCurrentEpochRequest.encode(request).finish();
    const promise = this.rpc.request("rollapp.epochs.types.Query", "CurrentEpoch", data);
  ...
  EpochInfo(request: QueryEpochInfoRequest): Promise<QueryEpochInfoResponse> {
    ...
    const promise = this.rpc.request("rollapp.epochs.types.Query", "EpochInfo", data);
