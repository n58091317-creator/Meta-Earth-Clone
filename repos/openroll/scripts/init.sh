...
CLIENT_FILE="$CONFIG_DIRECTORY/client.toml"
...
# --------------------- set syncer --------------------- #
SEQUENCER_ADDR=$($EXECUTABLE keys show rol-user --address --keyring-backend test)
jq --arg relayers "$SEQUENCER_ADDR" '.app_state.region.relayerList[0].address = $relayers' "$GENESIS_FILE" > "$tmp" && mv "$tmp" "$GENESIS_FILE"
echo "sync-hub-key-name = \"$KEY_NAME_ROLLAPP\"" >> "$CLIENT_FILE"
echo "sync-hub-key-address = \"$SEQUENCER_ADDR\"" >> "$CLIENT_FILE"
