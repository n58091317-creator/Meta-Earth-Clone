...
func networkWithRelayerObjects(t *testing.T, n int) (*network.Network, []types.Relayer) {
	...
	for i := 0; i < n; i++ {
		...
		nullify.Fill(&relayer)
		state.RelayerList = append(state.RelayerList, relayer)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.RelayerList
}

func TestShowRelayer(t *testing.T) {
	net, objs := networkWithRelayerObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowRelayer(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetRelayerResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.Relayer)
				require.Equal(t,
					nullify.Fill(&tc.obj),
					nullify.Fill(&resp.Relayer),
				)
			}
		})
	}
