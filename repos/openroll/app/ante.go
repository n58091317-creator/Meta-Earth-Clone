...
func GetAnteDecorators(options HandlerOptions) []sdk.AnteDecorator {
	...
	anteDecorators := []sdk.AnteDecorator{
		...
		NewDeductFeeDecorator(
			...
			options.regionKeeper,
			options.hubauthuKeeper,
			options.kycKeeper,
		),
...
// NewPostHandler returns an AnteHandler that checks and increments sequence
// numbers, checks signatures & account numbers, and deducts fees from the first
// signer.
func NewPostHandler(bandKeeper kyctypes.BankKeeper) (sdk.AnteHandler, error) {
	anteDecorators := []sdk.AnteDecorator{
		//kycModule.NewRefoundFee(bandKeeper, types.FeeCollectorName),
	}
